...
function autocomplete(input, latInput, lngInput) {
    ...
    dropdown.addListener('place_changed', function (_) {
...
function typeAhead(search) {
    ...
    searchInput.on('keyup', function (event) {
        ...
        var next = void 0;
        if (event.keyCode == 40 && current) next = current.nextElementSibling || items[0];else if (event.keyCode == 40) next = items[0];else if (event.keyCode == 38 && current) next = current.previousElementSibling || items[items.length - 1];else if (event.keyCode == 38) next = items[items.length - 1];else if (event.keyCode == 13 && current.href) return window.location = current.href;
